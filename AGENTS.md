...
本 repo 由 lore 维护多轴 wiki（99 页 · 8 个组件深度页 · 最后更新 2026-09-10）。
...
- **决策当下就记（agent 代捕获约定）**：做出非显然决策（选型 / 否决 / 权衡）时，追加一条 `kind:decision` 的 **draft** 原子——`why` 写清因果、`refs.anchors` 挂源码锚点 `fn @ file:line`：`lore note --draft --title "选 X 弃 Y" --why "因为 Z" --component <模块> --anchors "runAuto @ lib/runner.js:77"`（无 `lore` bin 时 `node <lore>/lib/cli.js note --draft …`）。draft = 机器代写；只有 owner 的直接动作能产生 confirmed。
- wiki 缺失（fresh clone：wiki 不进库）→ 先跑 `/lore-sync（codex 用户：/prompts:lore-sync）` 合成。
<!-- LORE_RESIDENT:END -->
