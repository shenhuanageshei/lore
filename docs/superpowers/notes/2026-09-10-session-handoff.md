...
1. ~~**推送**（阻塞项，非技术缺陷）~~ → **已于 2026-09-10 解除并完成**：GitHub 连接恢复，
   `git fetch` / `git push` 均成功；`v0.10.0` / `v0.11.0` 两个 tag 已上线，原 30 个提交全部推送。
   本轮壳工作又新增 5 个提交（`9bbd0b6`、`7472cdb`、`afa88f5`、`bb6d74b`、`34e4d09`）**尚未推送**——
   见 §4.3 的收口说明与计划 §9.4。
2. **壳的交互层缺浏览器实证**：见 §4.3；`headless Chrome` 在本环境产不出 DOM，
   F1–F5 点击 / F4 开关 / `[`/`]` 导航 / 窄屏视觉只有静态证据。
...
### 4.3 壳的视觉实现（**已于 2026-09-10 实施：阶段 A/B/C 完成，阶段 D 部分**）

> 更新（同日收口）：本节原写「未实施」。实际已由 `plans/2026-09-10-lore-shell-light-workbench.md`
> 落地上线，提交 `7472cdb`（A+B）→ `afa88f5` + `bb6d74b`（C）→ `34e4d09`（偏离审计修复轮）。
> **完整交付状态、实测证据、未完成项与待决策项见该计划 §9**——接手时以那里为准，本节只留索引。

设计文档 §5 已定 tokens（**浅色精密工作台**，Inter + JetBrains Mono，靛蓝 `#5B5BD6` / 青 `#0E7490`，弱色 `#6B7280` 4.83:1），原型在 `notes/2026-09-09-lore-shell-light.html`。已落进真实的 `site/index.html` + `site/shell.mjs`：
- 快捷键栏（F1–F5）+ **底部状态行**（可操作、显示燃料读数：决策捕获率 / 断流天数）——✅ 已落
- **源码锚点 = 引出线注记**（正文角标 + 右侧注记栏）——✅ 已落（覆盖面待决策，见计划 §9.4-2）
- **决策史 = 修改记录表**（版本 / 日期 / 变更与原因）——✅ 已落
- 无 JS / 窄屏降级——🟡 静态 HTML 与窄屏规则已验证；**交互层无浏览器实证**（环境无可用 headless 渲染）

**三条传下去的教训**（本轮实测得来）：
1. **验收探针必须验「行为」不是「字样」**：我曾据「F1–F5 字符串存在」宣布阶段 A 机检全绿，
   而它们其实是死键（`spec.run` 零调用）——独立审计才抓到。字样探针给的是虚假通过感。
2. **`GET /api/sync/status` 是热路径**：fuel 与 budget 两条派生各自同步 spawn git，
   实测每请求阻塞事件循环约 1s，而壳每 15s 轮询——加短 TTL 缓存后热态 5–7ms。
   新加任何派生读数前先问「它会不会 spawn 子进程」。
3. **eng_coder 单次 9 分钟墙钟，一次派 2 阶段必被掐**：本轮 6 次被掐，
   两次「A 完成 B 半途」、其余在被掐前已完成实质工作。节奏必须是
   **派活 → 复验自检 → 打检查点提交**，绝不等它自己跑完。
