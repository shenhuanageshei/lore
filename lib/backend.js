...
export function axisPrompt(page) {
  ...
  const userNote = String(page.instruction ?? '').trim();
  const USER = userNote ? [`【用户对本页的额外要求（在保持上述页面标准的前提下满足）】：${userNote}`] : [];
  return [...COMMON, ...(byAxis[page.axis] ?? byAxis.component), ...USER, ...TAIL].join('\n');
}

// ---------- 结果取用（S6 成本账本） ----------
// 契约仍是 Promise<string>；后端若能回报用量，可 resolve {text, tokens}（向后兼容的扩展）。
// 当前三个 CLI 后端都不回报 usage → tokensOf 返回 undefined → 账本不写 tokens 键（不伪造 0）。
// 之所以在这里给缝而不是在 runner 里猜：只有后端自己知道 CLI 有没有吐用量。
export function textOf(result) {
  return typeof result === 'string' ? result : String(result?.text ?? '');
}

export function tokensOf(result) {
  const t = (result !== null && typeof result === 'object') ? result.tokens : undefined;
...
function classify(name, err, stderr) {
  ...
  // maxBuffer 溢出同样置 killed=true——必须先判，否则被误报成 timeout（诊断误导：后端没坏只是输出超限）
  if (err.code === 'ERR_CHILD_PROCESS_STDIO_MAXBUFFER') return new BackendError(`${name} output exceeded maxBuffer`);
  ...
  return {
    ...
    rewritePage({ page, repoRoot, timeoutMs = 1_200_000 }) {
      ...
      return new Promise((resolveP, rejectP) => {
        // prompt 必须紧跟 -p：--allowedTools/--disallowedTools 是可变参数（<tools...>），
        // 放在它们之后会被当成工具名吞掉（claude 2.1.191 实测报 "Input must be provided..."，
        // auto 档每页必失败）。spec 2026-06-10 原始 argv 亦为 prompt 在前。
        exec('claude', ['-p', prompt, '--allowedTools', 'Read,Grep,Glob', '--disallowedTools', 'Write,Edit,Bash'],
    ...
    rewritePage({ page, repoRoot, timeoutMs = 1_200_000 }) {
      ...
      return new Promise((resolveP, rejectP) => {
        exec('codex', ['exec', prompt, '-C', repoRoot, '--sandbox', 'read-only', '--output-last-message', outFile],
          ...
          (err, stdout, stderr) => {
            if (err) {
              try { rmSync(outFile, { force: true }); } catch { /* best-effort：错误路径同样不留残留，防下一轮误当输出 */ }
              return rejectP(classify('codex', err, stderr));
            }
            let text = '';
...
// 20s 预算：CLI 冷启动实测可达 7.5s（claude 2.1.x 首次拉起），5s 会把装好的后端误判不可用
// ——探测只在 runner CLI 启动时跑一次，宽预算无副作用。
function probe(exec, cmd, args, timeoutMs = 20_000) {
