...
//   confirm <atom-id> [--why <w>] [--verdict confirm|dispute] [--by <who>]
//                                        追加一条确认记录（形状/派生在 lib/confirm.js）；
//                                        --by 缺省 unattributed——不署名就不冒充 owner（不变量⑦）
...
// 写入侧拒绝（AtomRejected / RecordRejected）与 ConfirmError 同列：可读消息 + exit 1，不打印栈。
...
import {
  ConfirmError, DEFAULT_CONFIRMED_BY, DEFAULT_VERDICT, VERDICTS, confirmAtom, confirmationSummary, deriveStatuses, readConfirmations,
} from './confirm.js';
import { AtomRejected, RecordRejected, readAllAtoms } from './journal.js';
...
verbs:
  ...
  confirm <atom-id> [--why <w>] [--verdict <v>] [--by <who>]
                                       确认一条原子（v = confirm|dispute，缺省 confirm；追加记录，不改原行）
                                       --by 缺省 unattributed（不署名就不冒充 owner）
...
options:
  ...
  --by <who>      confirm：署名者（缺省 unattributed——只有 owner 本人署名才写 owner；机器代写不得冒充）
...
function cmdConfirm({ journalDir, loreDir, args, rest, out, err }) {
  ...
  // 来源诚实（不变量⑦）：--by 缺省 unattributed，via 恒为 'cli'（本入口就是 CLI）；两者都落进确认记录。
  const r = confirmAtom(journalDir, {
    atom: atomId,
    verdict: args.verdict ?? DEFAULT_VERDICT,
    why: args.why ?? '',
    confirmedBy: args.by ?? DEFAULT_CONFIRMED_BY,
    via: 'cli',
  });
  const status = r.record.verdict === 'dispute' ? 'disputed' : 'confirmed';
  out('✓ confirm ' + atomId + ' → ' + status + ' · ' + r.record.id + ' · by ' + r.record.confirmed_by + label(r));
  return { code: 0 };
}
  ...
  try {
    switch (verb) {
      ...
      case 'confirm': {
        ...
        // --by 给了却没值 → 用法错误（不静默回落 unattributed：用户明确想署名时静默丢掉署名才是失真）
        if ('by' in args && !args.by) return badUsage('--by requires <who>');
    ...
    // AtomRejected / RecordRejected 同列（① 期遗留 B③）：写入被 schema / 记录层拒了是可预期的
    // 用户错误（如 --title 全是空白过得了存在性检查、过不了校验器），不是崩溃——
    // 与 lib/note.js / lib/mine.js 入口同一纪律，别让用户读到一段栈。
    if (e instanceof HumanStoreError || e instanceof ConfirmError
      || e instanceof AtomRejected || e instanceof RecordRejected) { err(`lore: ${e.message}`); return 1; }
    throw e;
  }
}
