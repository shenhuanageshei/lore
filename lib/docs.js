...
export function changelogExtractor(repoRoot) {
  ...
  const entries = [];
  // 兼容两种 changelog 风格：Keep-a-Changelog 的 `## [x.y.z] — date`，
  // 以及真实世界常见的 `## <标题> (date)` / `## <标题> — date`（无方括号版本号）。
  // 任意 `## 标题` 行：有 [版本] 或有日期才算一条；都无（如 `## 安装`）跳过。
  const re = /^##\s+(.+?)\s*$/gm;
  let m;
  while ((m = re.exec(text)) !== null) {
    const heading = m[1].trim();
    const vm = heading.match(/\[([^\]]+)\]/);
    const dm = heading.match(/(\d{4}-\d{2}-\d{2})/);
    if (!vm && !dm) continue;
    const version = vm ? vm[1]
      : heading.replace(/\(?\s*\d{4}-\d{2}-\d{2}\s*\)?/, '').replace(/[—–-]\s*$/, '').trim() || heading;
    entries.push({ version, date: dm ? dm[1] : '' });
  }
