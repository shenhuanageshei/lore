...
import { confirmationSummary } from './confirm.js';
...
//   confirmation S3 确认进度 {records, decisions, confirmed, disputed, pending}——决策原子的 owner 确认数。
...
export function diagnose(repoRoot, { now = new Date(), exec = execFileSync, gate = {} } = {}) {
  ...
  // 断流天数只算一次：报告字段与闸门判定必须同源（两处各算一遍就会漂移，评审 🔵#5）。
  const gapDays = j && j.lastHookTs ? daysBetween(j.lastHookTs, now) : UNKNOWN;
  // 窗口捕获率（S8 修订）：阈值解析只做一次，窗口大小与闸门判定同源。
  const thresholds = resolveGateThresholds(configText, gate);
  const win = j && Number.isInteger(thresholds.captureWindowCommits)
    ? gitWindow(root, thresholds.captureWindowCommits, { exec }) : null;
  const window_ = windowCapture(j, win, thresholds.captureWindowCommits);
...
export function formatReport(report) {
  ...
  lines.push(`  confirm    ${(cf && cf !== UNKNOWN)
    ? `已确认 ${cf.confirmed} · 待确认 ${cf.pending} · 已否决 ${cf.disputed}（决策 ${cf.decisions} · confirmation 记录 ${cf.records}）`
    : UNKNOWN}`);
