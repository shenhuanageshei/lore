...
// 剥 git trailer 噪音（Co-Authored-By / Signed-off-by 等 *-by: 行）。
// 只剥 `-by:` 家族：Note:/Fixes:/中文冒号等可能有信息量的行不动（YAGNI）。
// mine 在源头用（新原子 why 干净）；renderDecisionHistory 也用（防御已落盘的旧原子）。
const TRAILER_RE = /^[a-z][a-z0-9-]*-by:\s/i;
