...
import { CODE_EXT } from './source.js';
...
export function init({ repoRoot, srcSiteDir, home }) {
  ...
  // home 透传给 alignAssets（测试隔离机器层 hosts.json/宿主命令）；生产 undefined → 真 homedir()。
  const hadGitignore = existsSync(join(repoRoot, '.gitignore'));
  const actions = alignAssets(repoRoot, loreDir, { engineSiteDir: srcSiteDir, installHook: true, home });
  ...
  return {
    ...
    // resident 资产名三类全认：claude-md/agents-md（migrate RESIDENT_ASSET 映射表）、
    // resident:<file>（未来新 instructionFile 的 fallback 名）、mcp-json。
    resident: parseConfigResident(cfgText) && actions.some(a => a.asset === 'mcp-json' || a.asset.endsWith('-md') || a.asset.startsWith('resident:')),
    actions,
  };
}
