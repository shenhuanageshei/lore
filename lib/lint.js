...
// 设计 §3.4：不落 trailer-only 的 why，且「必须做成会红的测试，否则就是约定，约定必腐」；
// 计划 §3 全局验收：「trailer-only → lint/测试会红」。
// 口径（设计附录 B）：why 非空、但剥掉 Co-Authored-By / Signed-off-by / Generated with 后为空 = 「无正文」。
// 两个落点分别查——只查一处就留了安静的回退路径：
//   ① 已 finalize 的页（code_sha 盖章）：正文出现「只由 trailer 组成的行」= 无正文的 why 被物化进页。
//      剥哨兵区外的 fenced code / inline code span（机制档可能字面引用 commit message——同 lintUnfolded 的误报纪律）。
//      未 finalize 的页（无 code_sha）不查：它们还没走完 sync 的写入侧，同 lintUnfolded。
//   ② journal 原子（.lore/journal/**/*.ndjson）：写入侧 whyField 之后仍落进库的 trailer-only why = 写入路径回退。
//      历史存量（附录 B 实测 127 条）按 §3.4 不回填，但必须一直可见——「会红」的意义就在这里。
const TRAILER_LINE = line => line.trim() !== '' && stripTrailers(line) === '';
...
function ndjsonFiles(dir) {
  ...
  for (const e of entries) {
    ...
    if (e.isDirectory()) out.push(...ndjsonFiles(full));
...
export function lintTrailerOnly(loreDir) {
  const out = [];
  ...
  for (const axis of LINT_AXES) {
    ...
    for (const f of files) {
      let text = '';
      try { text = readFileSync(join(wikiDir, axis, f), 'utf8'); } catch { continue; }
      const { data, body } = parseFrontmatter(text);
      if (!data.code_sha) continue;                                  // 未 finalize → 不查（同 lintUnfolded）
      const bare = (body ?? '').replace(SENTINEL_RE, '').replace(FENCED_BLOCK_RE, '').replace(/`[^`\n]*`/g, '');
      bare.split('\n').forEach((line, i) => {
        if (TRAILER_LINE(line)) out.push({ kind: 'page', where: `${axis}/${f}`, line: i + 1, excerpt: oneLine(line) });
      });
    }
  }
  const journalDir = join(loreDir, 'journal');
  for (const file of ndjsonFiles(journalDir)) {
    let text = '';
    ...
    text.split('\n').forEach((line, i) => {
      ...
      if (!t) return;
      let atom;
      try { atom = JSON.parse(t); } catch { return; }                 // 坏行交给 doctor/readJournalStats 报，这里不重复
      if (atom === null || typeof atom !== 'object' || Array.isArray(atom)) return;
      const why = atom.why;
      if (typeof why !== 'string' || why.trim() === '' || stripTrailers(why) !== '') return;
      out.push({ kind: 'atom', where: atom.id ?? file, line: i + 1, excerpt: oneLine(why) });
    });
  }
  return out;
}
...
export function lint({ loreDir }) {
  ...
  const trailerOnly = lintTrailerOnly(loreDir);
  ...
  return {
    stale, orphans, missing, unfolded, trailerOnly, mermaid, missingDiagram, missingMechanism, deepConfig, themeDeep: themeDeepFatal, themeDeepWarnings,
    clean: stale.length === 0 && orphans.length === 0 && missing.length === 0 && unfolded.length === 0 && trailerOnly.length === 0 && mermaid.length === 0 && missingDiagram.length === 0 && missingMechanism.length === 0 && deepConfig.length === 0 && themeDeepFatal.length === 0,
  };
}
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  if (r.clean) {
    ...
    if (r.trailerOnly.length) console.log(`trailer-only (${r.trailerOnly.length}): ` + r.trailerOnly.map(t => `${t.where}:${t.line} "${t.excerpt}"`).join('; ') + ' → why 剥掉 trailer 后无正文（不变量⑦：无正文不写 why）');
