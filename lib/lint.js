...
export function lint({ loreDir }) {
  ...
  return {
    stale, orphans, missing, unfolded, mermaid, missingDiagram,
    clean: stale.length === 0 && orphans.length === 0 && missing.length === 0 && unfolded.length === 0 && mermaid.length === 0 && missingDiagram.length === 0,
  };
}
