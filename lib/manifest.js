...
import { extractSections } from './section.js';
...
export function emitManifest({
  ...
  for (const ax of axisDefs) {
    ...
    if (ROOT_AXIS_FILES[ax.id]) {
      ...
      if (ax.id === 'docs') {
        const GROUP_RANK = { '项目状态': 0, '设计': 1, '计划': 2, '架构': 3, '接口': 4, '调试': 5, '笔记': 6, '其它': 7 };   // 元文档置顶；未知 99 垫底；排序单一来源在此，壳不重排
        pages.sort((a, b) =>
          ((GROUP_RANK[a.group] ?? 99) - (GROUP_RANK[b.group] ?? 99))
...
function pageEntry(wikiDir, axisId, id, currentSha, countCommitsSince, language, staleScopes = {}, pageGroups = {}) {
  ...
  return {
    ...
    group: data.group ?? pageGroups[id] ?? '',          // frontmatter 优先（docs 物化写入）、map 兜底（component finalize 构造）
    paired_plan: data.paired_plan ?? '',
    sections: extractSections(body),                    // 节索引：ask 节级命中 / lore_page(section) 切片的数据源
    // 架构图质量门：component/flow 页 sync.md 要求 mermaid 架构图——缺了标记出来（壳徽标 + lint），
    // 防 sync 执行 agent 漏图默默接受（jpm 6 页全漏的根因）。theme/docs/HOME 不强制图，无此字段。
    ...((axisId === 'component' || axisId === 'flow') ? { hasDiagram: /```mermaid/.test(body) } : {}),
    // 两档质量门：深度页（component 轴 + 有分组 = config deep 声明的子模块）要完整两档——
    // 缺「## 机制详解」折叠档标记出来。鸟瞰页只要概览档、theme/flow 单档，不检测（无此字段）。
    ...((axisId === 'component' && pageGroups[id]) ? { hasMechanism: /^##\s+机制详解/m.test(body) } : {}),
  };
}

export function gitCurrentSha(repoRoot) {
  try {
    return execFileSync('git', ['rev-parse', '--short', 'HEAD'], { cwd: repoRoot, windowsHide: true })
      .toString().trim();
  } catch (e) {
...
export function makeCountCommitsSince(repoRoot) {
  return (sha, pathspec) => {
    try {
      ...
      const out = execFileSync('git', args, { cwd: repoRoot, stdio: 'pipe', windowsHide: true }).toString().trim();
      return Number(out) || 0;
    } catch {
