...
const TOOLS = [
  ...
  { name: 'lore_page', description: '取单个 wiki 页。id = page:<axis>/<id> 或 wiki 相对 path；section 参数只取单节；view="agent" 跳人读概览档 + 决策史物化区，只留机制骨架（省约 60% token；决策史改用 section="Decision history" 按需取）',
...
function callTool(name, args) {
  if (name === 'lore_ask') {
    if (!existsSync(join(wikiDir, '.manifest.json'))) {
      // fresh clone：wiki 不进库（facts-only 边界）——引导而非报错
