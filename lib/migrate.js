...
import { alignHostAssets, instructionFilesFor, readEnabledHosts, defaultHostsPath, HOST_NAMES } from './host.js';
...
// 已存在 → 收敛 refresh（**刷新域 = 引擎已知的全部 instruction file**，不看宿主是否启用）；
// 缺失 → 一次性首装（仅限启用宿主的文件；applied 记录后用户删除不复活）。
// 刷新域与启用域分离的理由：hosts.json 是**机器级**清单（与本 repo 无关），只刷启用宿主会让
// 先前由别的宿主装过的文件停在首装时的 stats（实测本机 hosts.json 空 → AGENTS.md 卡在 96 页，
// CLAUDE.md 已 97 页，同源漂移）。
...
export function alignResidentAssets(repoRoot, loreDir, mcpJsPath, applied, recordFn, { hostsPath } = {}) {
  ...
  const enabledFiles = instructionFilesFor(readEnabledHosts(hostsPath ?? defaultHostsPath()));
  const files = instructionFilesFor(HOST_NAMES);                    // 刷新域：HOSTS 声明的全部 instruction file
  ...
  for (const f of files) {
    ...
    } else if (enabledFiles.includes(f) && !covered(f)) {
      // 首装只发生在启用宿主的文件上——不把 instruction file 塞给从未要过它的 repo。
  ...
  if (enabledFiles.includes('CLAUDE.md')) {
...
// .lore/human/ = per-human 理解档案：默认不进 git（设计 §3.3 / 不变量⑥），可经 `lore human export` 导出。
export const GITIGNORE_LINES = ['.lore/.state/', '.lore/wiki/', '.lore/site/', '.lore/human/'];
...
export function alignAssets(repoRoot, loreDir, opts = {}) {
  ...
  if (parseConfigResident(cfgText)) {
    // hostsPath 随 opts.home 注入（测试隔离真机 hosts.json）；缺省 → defaultHostsPath() 真 homedir，
    // 与下一行 alignHostAssets({ home: opts.home }) 同一注入约定。
    guard('resident', () => alignResidentAssets(repoRoot, loreDir, mcpJsPath, applied, record, { hostsPath: defaultHostsPath(opts.home) }));
  }
