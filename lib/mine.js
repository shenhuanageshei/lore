...
import { appendAtom, AtomRejected, existingIds } from './journal.js';
...
export function mine({ repoRoot, journalDir, codeRoots, themes = [], flows = [], mines = ['commits'], now } = {}) {
  ...
  for (const a of atoms) {
    ...
    // S1：写入经 lib/journal.js 的校验闸门（非法原子抛 AtomRejected——本函数不吞它：
    // 挖掘器自己造出非法原子是 bug，静默跳过就是「安静地不工作」）。
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  let r;
  try {
    r = mine({ repoRoot, journalDir, codeRoots, themes, flows, mines });
  } catch (e) {
    // 校验器拒绝（S1）→ 可读错误 + exit 1（不是栈）。挖掘器造出非法原子 = 需要修的 bug。
