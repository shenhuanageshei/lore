...
export function autostart(action, { appData = process.env.APPDATA, platform = process.platform } = {}) {
  ...
  // node 用绝对路径（process.execPath）而非裸 `node`——开机登录时 Startup 的执行环境 PATH 往往不含 node，
  // 裸命令静默失败（实测重启后 portal 没起的真因）。vbs 字面量内每个 " 转义成 ""。
  const node = (process.execPath || 'node').replace(/\\/g, '/');
