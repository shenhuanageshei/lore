...
// LLM 只读三后端链（claude/codex/opencode，不可用类错误 failover）、runner 写盘；质量门在写盘前；append-only 任务历史。
...
import {
  readSyncConfig, readBudgetConfig, writeRunnerPid, clearRunnerPid, appendAutoRun, readRewriteRequests,
  runnerAlive, readAutoPending, clearAutoPending, readAutoRuns,
} from './syncstate.js';
import { isAlive } from './serve.js';
import { resolveBackendChain, textOf, tokensOf } from './backend.js';
import { appendCost, budgetStatus, versionStamp } from './cost.js';

// axisPrompt 已搬至 lib/backend.js（三后端共用 prompt 构造）——re-export 保既有 import 兼容。
export { axisPrompt } from './backend.js';

// ticker 判定：静默期 / schedule 到点 / 防叠跑 / 轮间冷却 / 预算闸，全在这一个纯函数（可测）。
// budget 由调用方（tickAuto）算好传入（lib/cost.js 的 budgetStatus）——本函数只读入参、不碰磁盘。
export function shouldRunAuto({ config, pendingTs, lastRunDate, lastRunTs, now, runnerAlive, budget = null }) {
  if (runnerAlive || config.mode !== 'auto') return { run: false, reason: null };
  // 预算闸（S6）：每版本 token 上限用尽 → 不启动，等人工放行（清 budget.json 或发新版本）。
  // 未配置预算 → budget 为 null → 与今天完全一致。
  if (budget?.exceeded) return { run: false, reason: 'budget' };
...
// 主流程：工单（rewrite 队列页置顶）→ 逐页串行后端链 → 质量门 → 写盘 → 历史 → finalize。
export async function runAuto(loreDir, { backend, fallbackBackends = [], maxPages, timeoutMs = 1_200_000, spawnFn = spawn, now = () => new Date() } = {}) {
  ...
  const costVersion = versionStamp(repoRoot);      // 每版本预算的窗口键（一次解析，整轮共用）
  ...
  try {
    ...
    for (const w of ordered) {
      const t0 = Date.now();
      // 后端链：仅「不可用类」错误（cli-missing/auth/provider）换下一个；timeout/质量门类不换（页的问题非后端问题）
      const chain = [backend, ...fallbackBackends];
      const tried = [];
      let newText = null, usedBy = null, lastErr = null;
      for (const b of chain) {
        const name = b.name ?? 'custom';
        tried.push(name);
        const callStart = Date.now();
        let out, callErr = null;
        try { out = await b.rewritePage({ page: w, repoRoot, timeoutMs }); } catch (e) { callErr = e; }
        // 每次 LLM 调用无条件记一行（成功/失败/超时都记；failover 的两个后端各记各的）。
        // tokens 只在后端回报时写入——CLI 后端不回报 → 键缺失（不伪造 0，见 lib/cost.js）。
        // 账本 I/O 单独兜底（评审 🔵#7）：账本是证据不是运行前提——磁盘满/权限/路径被占都不该让
        // 整轮重写陪葬（后面每页的结果、appendAutoRun 的轮次历史、finalize 都要照常走完）。
        try {
          appendCost(stateDir, {
            ts: now().toISOString(), page: w.path, backend: name, ms: Date.now() - callStart,
            ok: !callErr, ...(callErr ? {} : { tokens: tokensOf(out) }), version: costVersion,
          });
        } catch { /* best-effort：记账失败不中断本轮 */ }
        if (callErr) { lastErr = callErr; if (!callErr?.unavailable) break; continue; }
        newText = textOf(out); usedBy = name; lastErr = null; break;
      }
      if (lastErr) {
        // reason 带全链 tried——只记最后一个后端的错误会让人以为只有它挂（诊断零信息同源隐患）
        results.push({ page: w.path, ok: false, reason: `全后端不可用[${tried.join('→')}]: ${String(lastErr.message ?? lastErr)}`.slice(0, 120), ms: Date.now() - t0 });
        continue;
      }
      ...
      if (gate.ok) {
        ...
        results.push({ page: w.path, ok: true, backend: usedBy, ms: Date.now() - t0 });
      } else {
        // 格式类失败必须带新页开头片段：历史里只有「缺 frontmatter」时无从知道模型吐了什么——
        // 实测排查要重跑一整页（6 分钟）才看到真因，正是本仓反复记录的「诊断零信息」坑。
        const head = String(newText ?? '').replace(/\s+/g, ' ').trim().slice(0, 80);
        const reason = /frontmatter/.test(gate.reason) ? `${gate.reason} [head: ${head}]` : gate.reason;
        results.push({ page: w.path, ok: false, reason, backend: usedBy, ms: Date.now() - t0 });
      }
    }
  } finally {
...
export function tickAuto(loreDir, { spawnFn = spawn } = {}) {
  try {
    ...
    // 预算闸：只在显式配置了预算时才算（未配置 → null → shouldRunAuto 不看它）。
    // 维度取自 budget.json（tokens|calls|ms；缺省 calls——后端不回报 token 用量，token 维度算不出超支）。
    const budgetCfg = readBudgetConfig(stateDir);
    const budget = budgetCfg.budget == null ? null
      : budgetStatus({ stateDir, repoRoot: join(resolve(loreDir), '..'), budget: budgetCfg.budget, dimension: budgetCfg.dimension });
    ...
    // 超预算也走这里：不 spawn、不清 pending（pending 留着，放行后下一 tick 立刻跑）——「无副作用」
    if (!d.run) return d.reason ? { run: false, reason: d.reason } : { run: false };
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  const config = readSyncConfig(join(loreDir, '.state'));
  resolveBackendChain(config.backend)
    .then(chain => {
      if (!chain.length) {
        console.error('auto-rewrite: no available LLM backend (claude/codex/opencode 均未探测到)');
        process.exit(1);
      }
      return runAuto(loreDir, { backend: chain[0], fallbackBackends: chain.slice(1) });
    })
