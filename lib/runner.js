...
export function tickAuto(loreDir, { spawnFn = spawn } = {}) {
  try {
    ...
    // 预算闸：只在显式配置了 token_budget 时才算（未配置 → null → shouldRunAuto 不看它）
    const budgetCfg = readBudgetConfig(stateDir);
    const budget = budgetCfg.token_budget == null ? null
      : budgetStatus({ stateDir, repoRoot: join(resolve(loreDir), '..'), tokenBudget: budgetCfg.token_budget });
