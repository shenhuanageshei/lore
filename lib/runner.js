...
export async function runAuto(loreDir, { backend, fallbackBackends = [], maxPages, timeoutMs = 1_200_000, spawnFn = spawn, now = () => new Date() } = {}) {
  ...
  try {
    ...
    for (const w of ordered) {
      ...
      if (lastErr) {
        results.push({ page: w.path, ok: false, reason: String(lastErr.message ?? lastErr).slice(0, 120), ms: Date.now() - t0 });
        continue;
      }
      ...
      if (gate.ok) {
        ...
        results.push({ page: w.path, ok: false, reason: gate.reason, backend: usedBy, ms: Date.now() - t0 });
      }
    }
  } finally {
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  resolveBackendChain(config.backend)
    ...
    .then(r => { if (r) { console.log(`auto-rewrite: ${r.pages.filter(p => p.ok).length}/${r.pages.length} pages ok (${r.total_ms}ms)`); process.exit(0); } })
