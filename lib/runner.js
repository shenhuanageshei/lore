...
import { resolveBackendChain } from './backend.js';
  ...
  try {
    ...
    for (const w of ordered) {
      ...
      for (const b of chain) {
        tried.push(b.name ?? 'custom');
        try { newText = await b.rewritePage({ page: w, repoRoot, timeoutMs }); usedBy = b.name ?? 'custom'; lastErr = null; break; }
        catch (e) { lastErr = e; if (!e?.unavailable) break; }
      }
      if (lastErr) {
