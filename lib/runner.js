...
import {
  ...
  readRunnerPid, readAutoPending, clearAutoPending, readAutoRuns,
} from './syncstate.js';
  ...
  return {
    rewritePage({ page, repoRoot, timeoutMs = 600_000 }) {
      const prompt = [
        ...
        `读 ${page.codeRoot ?? page.sourceFile ?? '相关'} 的源码与现有页 .lore/wiki/${page.path}，`,
        '按 lore 的两档页面标准（概览档零黑话 + 机制档 <details> 折叠 + 锚点锚符号）重写整页。',
        '要求：保留 frontmatter 的 title/summary 结构；{{LORE_JOURNAL}} token 或已折叠的 LORE_JOURNAL 哨兵区原位保留；',
          ...
          (err, stdout) => {
            if (err) {
              return rejectP(new Error(
                ...
                : err.killed ? 'timeout'
                : `claude exit: ${String(err.message ?? '').slice(0, 80)}`));
            }
...
export async function runAuto(loreDir, { backend, maxPages, timeoutMs = 600_000, spawnFn = spawn, now = () => new Date() } = {}) {
  ...
  try {
    ...
    const ordered = [                                      // user-requested 置顶（队列序），其余按 plan 序；去重
