...
export function axisPrompt(page) {
  ...
  const byAxis = {
    ...
    HOME: [
      '这是全仓认知入口页：一句话定位 → 状态节只输出单行 {{LORE_HOME_STATUS}}（不要搬运物化区，finalize 会重新填充）→ Knowledge flow mermaid → 「理解项目/排查问题/决策与时间线」三个 wikilink 导航节。',
    ],
  };
...
export async function runAuto(loreDir, { backend, maxPages, timeoutMs = 600_000, spawnFn = spawn, now = () => new Date() } = {}) {
  ...
  try {
    ...
    const queuedItems = queued.map(p =>
      worklist.find(w => w.path === p) ?? { path: p, axis: pathAxis(p), reason: 'user-requested' });
    const seen = new Set();
    const ordered = [];
