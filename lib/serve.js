...
// start() re-rolls the port up to this many times when its child loses the bind race
// to another process under parallel load (see waitForServer). 3 = the original single
// attempt plus two cheap retries; a child that lost the race is detected in ~tens of ms.
...
// 默认 node：控制 API + auto ticker + no-cache 都只有 node server 有，且 serve.js 本身就在
// node 里跑——python 兜底永远不该是隐式默认（曾导致控制台全灰 + 浏览器缓存旧页 + auto 档失效）。
// python 仅显式 --python opt-in；要了但没有 → 仍回 node。
...
export async function start({ loreDir, port, canRun, spawnFn = spawn, now = new Date().toISOString(), preferPython = false }) {
  ...
  // 双语 repo 的写 API 只有 node 有——即使显式 --python 也强制回 node。
  ...
  const runtime = probeRuntime(can, { python: preferPython && language.available.length <= 1 });

  // findPort→spawn→bind is a TOCTOU window: under parallel load another process can
  // claim the just-probed ephemeral port before our detached child binds it, crashing
  // the child (server.js has no listen-error handler). A bare port probe can't tell our
  // child from the thief now sitting on that port — so waitForServer verifies OUR child
  // is alive, and on a lost race ('died') we re-roll the port instead of trusting it.
  let lastErr = 'unknown';
  for (let attempt = 0; attempt < START_ATTEMPTS; attempt++) {
    ...
    const status = await waitForServer(chosen, child.pid);
    if (status === 'ready') {
      return { ...info, url: `http://127.0.0.1:${chosen}/site/`, reused: false };
    }
    clearPid(stateDir);
    if (isAlive(child.pid)) killPid(child.pid, process.platform);   // never orphan an alive-but-unbound child
    if (status === 'timeout') {                                     // alive yet never bound → broken server, don't retry
      throw new Error(`lore: server (pid ${child.pid}) did not bind on port ${chosen} within ~3s`);
    }
    lastErr = `server child (pid ${child.pid}) exited before binding port ${chosen}`;   // 'died' → retry fresh port
  }
  throw new Error(`lore: ${lastErr}; gave up after ${START_ATTEMPTS} attempts`);
}
...
// Resolve once we know whether OUR child is the one serving on `port`:
//   'ready'   — port accepts a connection AND our child (pid) is still alive
//   'died'    — our child is gone (crashed / lost the bind race) → caller re-rolls the port
//   'timeout' — child alive but never bound within ~3s (a genuinely broken server)
// The isAlive checks are what make this race-safe: a port probe alone cannot distinguish
// our child from another process that grabbed the same ephemeral port. Telling 'died'
// (transient, retry) apart from 'timeout' (broken, fail fast) keeps the no-bind path fast.
function waitForServer(port, pid, tries = 50) {
  return new Promise((resolve) => {
    const attempt = (n) => {
      if (pid != null && !isAlive(pid)) return resolve('died');
      const s = net.connect(port, '127.0.0.1');
      s.once('connect', () => { s.destroy(); resolve(pid == null || isAlive(pid) ? 'ready' : 'died'); });
      s.once('error', () => {
        s.destroy();
        if (pid != null && !isAlive(pid)) return resolve('died');
        if (n <= 0) return resolve('timeout');
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  (async () => {
    if (sub === 'start') {
      ...
      // --node 现为默认行为，保留旗标兼容（no-op）；--python 显式要静态 python（功能残缺，慎用）
      ...
      if (info.runtime === 'python') {
        console.log('  ⚠ python 静态模式：无控制台 API / 无 auto ticker / 无缓存头——仅在特殊场景使用');
      }
    } else if (sub === 'stop') {
