...
export function resolveConfiguredDeep(repoRoot, codeRoots, deep) {
  ...
  for (const [deepRoot, { order = [] }] of Object.entries(deep)) {
    // 空 order = 没声明任何模块 → 无可解析、根校验无意义。parseConfigDeep 全局扫 `deep:` 会把
    // axes.theme.deep 的父主题行扫成空 order 根（theme.deep 子记录是 `- { id }` 行，parseConfigDeep
    // 不解析）→ 此处跳过，绝不让 theme 父主题误报 deep-root-invalid。
...
function resolveThemeDeepGlob(repoRoot, entry, norm) {
  try {
    // core.quotePath=false：非 ASCII 文件名原样输出（默认 octal 转义会损坏中文路径）
    const out = execFileSync('git',
      ['-c', 'core.quotePath=false', 'ls-files', '--cached', '--others', '--exclude-standard', `:(glob)${norm}`],
    ...
    // 规则 9：逐文件 realpath 校验，拒绝落出仓库的 symlink/junction（git 不拦逃逸符号链接）
    const repoPath = resolve(repoRoot);
    const repoReal = realpathSync(repoPath);
    for (const f of files) {
      const canon = canonicalizeRepoFile(repoPath, f);
      if (canon === null) continue;   // 无法解析（已删文件等）——放行，交给后续读盘
      if (!pathContains(repoReal, canon)) {
        return { status: 'outside-repo', entry, resolvedPath: relative(repoRoot, canon).replaceAll('\\', '/') };
      }
    }
...
export function resolveThemeDeepSources(repoRoot, entries) {
  ...
  if (all.length > THEME_DEEP_MAX_CHILD_SOURCES) {
    return { status: 'invalid', entry: '', reason: `child expanded to ${all.length} files (max ${THEME_DEEP_MAX_CHILD_SOURCES})` };
  }
  const seen = new Set();
  const files = [];
...
export function resolveConfiguredThemeDeep(repoRoot, themes, themeDeep) {
  ...
  if (totalFiles > THEME_DEEP_MAX_TOTAL_FILES) {
    // 预算超限与 per-child 同构：fail-closed，丢弃全部子（Task 4 不得据此建工单）
