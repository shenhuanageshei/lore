...
function validDeepRoot(repoRoot, deepRoot, codeRoots) {
  ...
  let repoReal, deepReal;
  try {
    repoReal = realpathSync(repoPath);
    deepReal = realpathSync(deepPath);
  } catch { return false; }
  ...
  return codeRoots.some(cr => {
    ...
    try {
      const codeReal = realpathSync(codePath);
      return pathContains(repoReal, codeReal) && pathContains(codeReal, deepReal);
    } catch { return false; }
  });
}
