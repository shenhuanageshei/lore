...
import { resolveDeepSource } from './source.js';
...
function canonicalizeFromExisting(path) {
  ...
  try {
    ...
    if (suffix.length && !lstatSync(real).isDirectory()) return null;
...
function validDeepRoot(repoRoot, deepRoot, codeRoots) {
  ...
  if (!deepReal) return false;
  if (!pathContains(repoReal, deepReal)) return false;
  return codeRoots.some(cr => {
    ...
    return codeReal !== null
      && pathContains(repoReal, codeReal)
      && pathContains(codeReal, deepReal);
  });
}
