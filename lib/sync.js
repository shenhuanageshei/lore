...
// 决策史小节标题的判定——**口径必须与 site/shell.mjs 的「决策史小节标题唯一口径」一致**
// （遗留 🔵#10：三处独立判定，此前只有壳的 F3 判定带 /i，其余两处大小写敏感）：
//   · 变体集：英文 `Decision history` + 决策史 / 决策历史 / 决策与时间线；
//   · **大小写不敏感**——这正是本轮修的不一致：小写 `## decision history` 会让壳的 F3 亮起、
//     却既不转表也不被 fold 认到；
//   · 标题名后必须是**空白或行尾**：`## Decision historybook` / `## Decision history中文` /
//     `## 决策历史附录` 这类 lookalike **不是**决策史小节（下方 lookalike 用例钉住它们），
//     壳侧必须给出同一个答案，故它那边从「前缀匹配」收紧到同一条边界规则。
//   · 顺带统一：`决策史/决策历史/决策与时间线` 现在同样允许「空白 + 尾注」（旧式只给
//     `决策历史 (Decision history)` 开了特例）——两侧同宽松，规则才真的是一份。
// 为什么这里不 import site/shell.mjs 的常量：分层方向是 壳 → lib（site/ 是物化产物，可被覆盖），
//   lib 反向依赖 site/ 会缠住初始化顺序。两处一致性改由 test/shell.test.js 的**交叉断言**守住：
//   同一样本集分别喂 foldJournal 与本文件外的壳侧判定，逐条比对结论，谁改歪了都会红。
const DH_HEADING_RE = /^##[ \t]+(?:Decision history|决策史|决策历史|决策与时间线)(?:[ \t][^\r\n]*)?\r?\n/mi;
