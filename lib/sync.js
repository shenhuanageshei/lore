...
import { refreshResident, residentStats } from './resident.js';
...
export function finalizeSync(loreDir, now, { warn = (m) => console.error(m) } = {}) {
  ...
  // resident 节刷新：CLAUDE.md 已有标记节才更新（用户删了 = 尊重，不复活）。best-effort。
  try { refreshResident(join(resolve(loreDir), '..'), residentStats(loreDir)); } catch { /* 不挡 finalize */ }
