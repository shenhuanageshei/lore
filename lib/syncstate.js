...
const BACKENDS = new Set(['auto', 'claude', 'codex', 'opencode']);
const CONFIG_DEFAULTS = { mode: 'notify', debounce_minutes: 10, schedule: null, max_pages: 5, stale_threshold: 15, backend: 'auto' };
...
export function readSyncConfig(stateDir) {
  ...
  return {
    ...
    // auto 档 LLM 后端：'auto' = 探测链（claude→codex→opencode），具体名 = 锁死单后端（无 fallback）
    backend: BACKENDS.has(raw.backend) ? raw.backend : CONFIG_DEFAULTS.backend,
  };
}
...
export function writeSyncConfig(stateDir, patch) {
  ...
  if ('backend' in patch && !BACKENDS.has(patch.backend)) throw new Error(`lore: invalid backend (auto|claude|codex|opencode)`);
  const p = join(stateDir, MODE_FILE);
  let raw = {};
  if (existsSync(p)) { try { raw = JSON.parse(readFileSync(p, 'utf8')) ?? {}; } catch { raw = {}; } }
  const next = { ...raw };
  for (const k of ['debounce_minutes', 'schedule', 'max_pages', 'stale_threshold', 'backend']) if (k in patch) next[k] = patch[k];
  if (next.schedule === null) delete next.schedule;        // null = 清空回默认，不留键
  mkdirSync(stateDir, { recursive: true });
  writeFileSync(p, JSON.stringify(next, null, 2) + '\n');
}

// --- 成本预算（S6）：每版本上限 + 计量维度；独立文件 budget.json，不塞进 sync.json（形状不动） ---
// 形状 {budget: number, dimension: 'tokens'|'calls'|'ms'}；budget 缺键/null = 未配置 = 不阻断
// （与今天行为完全一致）。dimension 缺省 = calls（审计 D3：后端不回报 token 用量，token 维度算不出超支）。
// 旧文件 {token_budget: n} 仍可读——等价于 {budget: n, dimension: 'tokens'}（不静默改既有语义）。
// 账本与求和口径在 lib/cost.js（维度词表从那里 import，不复制一份）。
const BUDGET_FILE = 'budget.json';
export const BUDGET_DEFAULTS = Object.freeze({ budget: null, dimension: DEFAULT_BUDGET_DIMENSION });

export function readBudgetConfig(stateDir) {
  const p = join(stateDir, BUDGET_FILE);
  let raw = {};
  if (existsSync(p)) { try { raw = JSON.parse(readFileSync(p, 'utf8')) ?? {}; } catch { raw = {}; } }
  const legacy = !('budget' in raw) && raw.token_budget !== undefined;
  const b = legacy ? raw.token_budget : raw.budget;
  const dimension = legacy ? 'tokens'
    : (BUDGET_DIMENSIONS.includes(raw.dimension) ? raw.dimension : DEFAULT_BUDGET_DIMENSION);
  // 非法值回「未配置」（同 readSyncConfig 的 best-effort 口径）——坏配置不该把 ticker 打崩
  return { budget: Number.isFinite(b) && b > 0 ? b : null, dimension };
}

// patch: {budget, dimension}；budget:null = 清空（不留键）；token_budget 是旧别名（= budget + tokens 维度）。
export function writeBudgetConfig(stateDir, patch = {}) {
  const p = join(stateDir, BUDGET_FILE);
  let raw = {};
  if (existsSync(p)) { try { raw = JSON.parse(readFileSync(p, 'utf8')) ?? {}; } catch { raw = {}; } }
  const next = { ...raw };
  const legacyKey = 'token_budget' in patch;
  const hasBudget = 'budget' in patch || legacyKey;
  const cap = legacyKey ? patch.token_budget : patch.budget;
  if (hasBudget && cap !== null && !(Number.isFinite(cap) && cap > 0)) {
    throw new Error('lore: invalid budget (positive number, or null to clear)');
  }
  if ('dimension' in patch && !BUDGET_DIMENSIONS.includes(patch.dimension)) {
    throw new Error('lore: invalid budget dimension (' + BUDGET_DIMENSIONS.join('|') + ')');
  }
  if (hasBudget) {
    delete next.token_budget;                                    // 旧键一律清掉，只留新形状
    if (cap === null) {                                          // null = 清空回「不阻断」，不留键
      delete next.budget; delete next.dimension;
    } else {
      next.budget = cap;
      next.dimension = legacyKey ? 'tokens'
        : ('dimension' in patch ? patch.dimension : (next.dimension ?? DEFAULT_BUDGET_DIMENSION));
    }
  } else if ('dimension' in patch) {
...
// runner 一轮绝对上限（max_pages × per-page timeout 的保守上界）——超过此龄的 pid 必是残留/卡死，不再认作「在跑」。
// 防 pid 复用死锁：机器重启后旧 runner 的 pid 被新进程占用，isAlive 误判 true 会让 ticker 永不触发（实测踩过）。
// 取 180min 覆盖 max_pages≤9 × per-page 1200s（runner.js timeout）。调大 per-page timeout 必同步抬此值，否则长轮 runner 被误判过期、ticker 启第二个。
export const RUNNER_STALE_MS = 180 * 60 * 1000;
