...
// --- 成本预算（S6）：每版本 token 上限；独立文件 budget.json，不塞进 sync.json（mode/config 形状不动） ---
// null / 缺键 = 未配置 = 不阻断（与今天行为完全一致）；账本与求和口径在 lib/cost.js。
const BUDGET_FILE = 'budget.json';
export const BUDGET_DEFAULTS = Object.freeze({ token_budget: null });

export function readBudgetConfig(stateDir) {
  const p = join(stateDir, BUDGET_FILE);
  let raw = {};
  if (existsSync(p)) { try { raw = JSON.parse(readFileSync(p, 'utf8')) ?? {}; } catch { raw = {}; } }
  const b = raw.token_budget;
  // 非法值回「未配置」（同 readSyncConfig 的 best-effort 口径）——坏配置不该把 ticker 打崩
  return { token_budget: Number.isFinite(b) && b > 0 ? b : null };
}

export function writeBudgetConfig(stateDir, patch) {
  if ('token_budget' in patch && patch.token_budget !== null
      && !(Number.isFinite(patch.token_budget) && patch.token_budget > 0)) {
    throw new Error('lore: invalid token_budget (positive number, or null to clear)');
  }
  const p = join(stateDir, BUDGET_FILE);
  let raw = {};
  if (existsSync(p)) { try { raw = JSON.parse(readFileSync(p, 'utf8')) ?? {}; } catch { raw = {}; } }
  ...
  if ('token_budget' in patch) {
    if (patch.token_budget === null) delete next.token_budget;   // null = 清空回「不阻断」，不留键
    else next.token_budget = patch.token_budget;
  }
  mkdirSync(stateDir, { recursive: true });
  writeFileSync(p, JSON.stringify(next, null, 2) + '\n');
}
